...
export function PlayerHUD() {
  ...
  const armorPct = Math.min((player.armorPointBalance / 100) * 100, 100);
  const weaponPct = Math.min((player.weaponPointBalance / 100) * 100, 100);
  const corePct = 42; // Neuoral Sync mock
  ...
  return (
    <div className="panel" style={{ position: "relative" }}>
      <div className="panel-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <span style={{ color: "var(--accent)", fontSize: "12px" }}>{(t.dashboard.activeStatus || "ACTIVE").toUpperCase()}</span>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 12h-4l-3 9L9 3l-3 9H2" /></svg>
        </div>
      </div>

      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "24px 0" }}>
        <div style={{ alignSelf: "center", width: "160px", height: "200px", background: "linear-gradient(to bottom, rgba(120, 214, 107, 0.1), transparent)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: "16px", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", marginBottom: "32px", overflow: "hidden" }}>
          <img
            src="/hero.png"
            alt="Character Preview"
            style={{ width: "100%", height: "100%", objectFit: "cover", filter: "drop-shadow(0 0 20px rgba(16, 185, 129, 0.3))" }}
          />
          <div style={{ position: "absolute", bottom: "-16px", width: "120px", height: "16px", background: "rgba(120, 214, 107, 0.3)", filter: "blur(15px)", borderRadius: "50%" }}></div>
        </div>
...
function StatBar({ label, current, max, color, pct }: { label: string, current: number, max: number, color: string, pct: number }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: "10px", fontWeight: "bold", textTransform: "uppercase" }}>
        <span style={{ color: "var(--muted)" }}>{label}: {current.toFixed(1)} / {max}</span>
