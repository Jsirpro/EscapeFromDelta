...
#[derive(Accounts)]
pub struct SettleFailedRaid<'info> {
    #[account(mut)]
...
pub fn handler<'info>(
    ...
    let retained_assets = raid_session.safe_case_selection.clone();
    let lost_assets = raid_session
        .carried_loot
        .iter()
        .copied()
    ...
    battle_record.result = if timed_out { RaidResult::TimedOut } else { RaidResult::Failed };
    battle_record.retained_assets = retained_assets;
    battle_record.lost_assets = lost_assets;
